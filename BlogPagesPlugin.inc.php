...
/**
 * @file plugins/generic/blogPages/BlogPagesPlugin.inc.php
 *
 ...
 * @class BlogPagesPlugin
 *
 * BlogPagesPlugin class
 *
 */

import('lib.pkp.classes.plugins.GenericPlugin');

class BlogPagesPlugin extends GenericPlugin {
	function getDisplayName() {
		return __('plugins.generic.blogPages.displayName');
	}

	function getDescription() {
		$description = __('plugins.generic.blogPages.description');
		if ( !$this->isTinyMCEInstalled() )
			$description .= "<br />".__('plugins.generic.blogPages.requirement.tinymce');
		return $description;
	}
	...
	function callbackHandleContent($hookName, $args) {
		...
		if ( $page == 'pages' ) {
			...
			define('HANDLER_CLASS', 'BlogPagesHandler');
			$this->import('BlogPagesHandler');
			return true;
		}
		return false;
	}
	...
	function getManagementVerbs() {
		...
		if ($this->getEnabled()) {
			if ($this->isTinyMCEInstalled()) {
				$verbs[] = array('settings', __('plugins.generic.blogPages.editAddContent'));
			}
		}
	...
	function manage($verb, $args, &$message, &$messageParams) {
		...
		switch ($verb) {
			case 'settings':
				...
				$this->import('BlogPagesSettingsForm');
				$form = new BlogPagesSettingsForm($this, $journal->getId());
			...
			case 'add':
				...
				$this->import('BlogPagesEditForm');

				$blogPageId = isset($args[0])?(int)$args[0]:null;
				$form = new BlogPagesEditForm($this, $journal->getId(), $blogPageId);
				...
				if (Request::getUserVar('edit')) {
					...
					if ($form->validate()) {
						...
						$templateMgr->assign(array(
							...
							'pageTitle' => 'plugins.generic.blogPages.displayName',
							'pageHierarchy' => $pageCrumbs,
							'message' => 'plugins.generic.blogPages.pageSaved',
			...
			case 'delete':
				...
				$blogPagesDao->deleteBlogPageById($blogPageId);

				$templateMgr->assign(array(
					'currentUrl' => Request::url(null, null, null, array($this->getCategory(), $this->getName(), 'settings')),
					'pageTitle' => 'plugins.generic.blogPages.displayName',
					'message' => 'plugins.generic.blogPages.pageDeleted',
					'backLink' => Request::url(null, null, null, array($this->getCategory(), $this->getName(), 'settings')),
					'backLinkLabel' => 'common.continue'
				));
